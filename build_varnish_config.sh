...
#      VARNISH_DOCKER_PORT
# default filter "expose=8080"
#
# usage build_varnish_config.sh > default.vcl

if [ -z $VARNISH_DOCKER_PORT ]; then
    VARNISH_DOCKER_PORT = '8080'
fi

# get all running docker container IDs
# filtered by VARNISH_DOCKER_FILTER
# default value: "expose=8080"
if [ -z "$VARNISH_DOCKER_FILTER" ]; then
        echo "################### WARNING ######################"
    echo "#     $VARNISH_DOCKER_FILTER is not set          #"
        echo "#                                                #"
        echo "# setting filter to default value \"expose=${VARNISH_DOCKER_PORT}\"  #"
        echo "################### WARNING ######################"
    container_list=$(docker ps --filter "expose=${VARNISH_DOCKER_PORT}" --format '{{.ID}}');
else
...
declare -A AG_BACKENDS
for container_id in $container_list; do
    declare -A BACKEND
    container_ip_list=`docker inspect --format '{{range $key, $value := .NetworkSettings.Networks}}{{println $value.IPAddress}}{{end}}' ${container_id}`
    BACKEND_IP=($container_ip_list[0])
    container_name=`docker inspect --format '{{ .Name}}' ${container_id}`
    container_name=${container_name//-/_} # convert to Varnish/VCL-friendly format
    BACKEND_NAME=${container_name#?}
    BACKEND = ([name]=${BACKEND_NAME}, [IP]=${BACKEND_IP})
    AG_BACKENDS = ("${AG_BACKENDS[@]}" BACKEND)
done

export AG_BACKENDS

# Everything after ##END is a {{mustache}} template
# https://mustache.github.io/
# {{mustache}} rendered us Bash implmentation
# https://github.com/tests-always-included/mo
# assumed to be installed in the same directory

. /usr/local/bin/mo
sed '0,/^###END###/ d' "$(basename "$0")" | /usr/local/bin/mo
###END###
vcl 4.0;

# Based on:
# https://github.com/mattiasgeniar/varnish-6.0-configuration-templates
# Autogenerated by {{AG_USERNAME}} at {{AG_DATE_TIME}}
...
# add declaration for each container
# selected by VARNISH_DOCKER_FILTER
{{#BACKENDS}}
backend {{.name}} { # Define one backend
    .host = "{{.IP}}";    # IP or Hostname of backend
    .port = "{{VARNISH_DOCKER_FILTER}}";    # Port http server is listening on
...
# add BACKENDS to round-robin director
# selected by VARNISH_DOCKER_FILTER
sub vcl_init {
